...
The above step gives us an idea that the increase in number errors as well as the response time might be due to the other app component ie. Cosmos DB. Let us examine it to make sure that is the case. 



When we open the database tab and expand the metrics pane related to the Cosmos Db, we see that the Ru has been consumed and that might be causing the throttling of by cosmos db.
...
Click on the Scale and settings option and set the value to 2400 RU and click on save present on the top of the blade. You will be able to see the associated cost too.
